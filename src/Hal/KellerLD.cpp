...
pressure KellerLD::getPressure()
{
	read();
	return P_bar * 1000.0f;
}

double KellerLD::getPressureConversion(float conversion)
{
	return P_bar * 1000.0f * conversion;
}

temperature KellerLD::getTemp()
{
	read();
	return T_degc;
}

depth KellerLD::getDepth()
{
	read();
	return (getPressureConversion(KellerLD::Pa) - 101325) / (fluidDensity * 9.80665);
}
